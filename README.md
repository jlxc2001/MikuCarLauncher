...
## v65
- 新增“界面比例缩放 / 车机屏幕适配”设置页，入口：
  - 我的 → 车机桌面设置 → 界面比例缩放 / 车机屏幕适配
- 支持独立调整：
  - 横向缩放 X：0.70~1.30
  - 纵向缩放 Y：0.70~1.30
  - 横向偏移 X：-520~520
  - 纵向偏移 Y：-180~180
- 背景图片不跟随 UI 缩放，仍由用户自行导入适合自己车机比例的背景图。
- LauncherCanvasView 绘制逻辑拆分：
  - 背景层保持原样。
  - 左侧按钮栏、首页卡片、应用抽屉、我的页面、图标、文字、选中框、卡片内容统一进入 UI 缩放变换。
- 触摸坐标加入 UI 缩放反向换算，缩放后左侧按钮、卡片、应用抽屉点击区域仍能对齐。
- 1号卡片高德地图 AppWidget 的位置和大小跟随 UI 缩放。
- Live2D 的首页显示中心点、缩放和遮罩裁切线跟随 UI 缩放；拖动/捏合调整页也同步参考缩放后的卡片位置。
- 调整页模拟卡片位置跟随 UI 缩放，便于不同车机比例下对齐模型。
- 保留 v64 的 Live2D 不闪屏、夜间变暗、画质/帧率、按钮层级修复等逻辑。
- 已保留 V64 稳定备份，方便大规模改动出问题时回溯。


## v66
- 在 v65 手动缩放基础上新增“自动适配当前屏幕比例”。
- 默认开启自动适配：
  - 根据真实屏幕宽高计算自动缩放。
  - 尽量保持 32:9 UI 等比完整显示。
  - 自动居中 UI，避免不同车机比例下卡片被横向/纵向拉伸。
- “界面比例缩放 / 车机屏幕适配”设置页新增：
  - 自动适配当前屏幕比例开关。
  - 手动微调 X/Y：在自动适配基础上叠加。
  - 手动偏移 X/Y：在自动居中基础上叠加。
  - 预设：自动适配 + 无手动微调。
  - 预设：关闭自动适配，使用手动模式。
- 背景图片仍不参与 UI 自适应缩放，用户可以继续导入适合自己车机比例的背景图。
- 左侧按钮、首页卡片、应用抽屉、我的页面、AppWidget、Live2D、调整页模拟卡片均继续跟随最终 UI 缩放结果。
